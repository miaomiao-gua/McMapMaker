...
# ========== Minecraft 1.20.1 map color palette ==========
BASE_COLORS = [
...
# ========== Frame loading (no frame limit) ==========
def load_frames_from_file(path, log_func=None):
    if log_func is None:
        log_func = print
    ext = os.path.splitext(path)[1].lower()
    log_func(f"File extension: {ext}")

    if ext in ('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp'):
        img = Image.open(path).convert("RGB")
        log_func(f"Static image size: {img.size}")
        return [img]
        ...
        try:
            ...
            log_func(f"GIF total frames: {n_frames if n_frames else 'unknown'}")
            frames = []
            for frame in ImageSequence.Iterator(gif):
                frames.append(frame.copy().convert("RGB"))
            gif.close()
            if not frames:
                log_func("Warning: no frames extracted, falling back to static")
                return [Image.open(path).convert("RGB")]
            log_func(f"GIF loaded frames: {len(frames)}")
            return frames
        except Exception as e:
            log_func(f"GIF error: {e}")
            raise
        ...
        try:
            ...
            if not cap.isOpened():
                raise RuntimeError("Cannot open video. Check path or codec.")
            total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            log_func(f"Video total frames: {total}")
            frames = []
            count = 0
            while True:
                ret, frame = cap.read()
                ...
                count += 1
                if count % 100 == 0:
                    log_func(f"Loaded {count} frames...")
            cap.release()
            log_func(f"Video actual frames loaded: {len(frames)}")
            return frames
        except Exception as e:
            log_func(f"Video error: {e}")
            raise
    else:
...
class MapConverterGUI:
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Map Converter", font=("Arial", 16)).pack(pady=10)

        f1 = tk.Frame(self.root); f1.pack(fill="x", padx=20, pady=2)
        tk.Label(f1, text="Input File:", width=10).pack(side="left")
        tk.Entry(f1, textvariable=self.file_path, width=30).pack(side="left", expand=True, fill="x")
        tk.Button(f1, text="Browse...", command=self.browse_file).pack(side="right")

        f2 = tk.Frame(self.root); f2.pack(fill="x", padx=20, pady=2)
        tk.Label(f2, text="Output Folder:", width=10).pack(side="left")
        ...
        tk.Checkbutton(f3, text="Lock Map", variable=self.locked).pack(side="left", padx=5)
        tk.Checkbutton(f3, text="Track Player Position", variable=self.tracking).pack(side="left", padx=5)

        f_sat = tk.Frame(self.root); f_sat.pack(fill="x", padx=20, pady=5)
        tk.Label(f_sat, text="Saturation:").pack(side="left")
        ...
        tk.Checkbutton(f_split, text="Split into Tiles", variable=self.split_enabled).pack(side=tk.LEFT, padx=5)
        tk.Label(f_split, text="Columns:").pack(side=tk.LEFT, padx=(10,0))
        tk.Entry(f_split, textvariable=self.split_cols, width=4).pack(side=tk.LEFT, padx=2)
        tk.Label(f_split, text="Rows:").pack(side=tk.LEFT, padx=(5,0))
        ...
        tk.Label(f_id, text="Start Map ID:").pack(side=tk.LEFT)
        tk.Entry(f_id, textvariable=self.start_id, width=6).pack(side=tk.LEFT, padx=5)
        tk.Label(f_id, text="(incremental)").pack(side=tk.LEFT)

        f_dither = tk.Frame(self.root); f_dither.pack(fill="x", padx=20, pady=5)
        tk.Label(f_dither, text="Dithering Mode:").pack(side=tk.LEFT)
        dither_opts = [("None", "none"), ("2x2 Dither", "2x2")]
        ...
        tk.Button(f_frame, text="◀ Previous", command=self.prev_frame).pack(side=tk.LEFT, padx=2)
        tk.Button(f_frame, text="Next ▶", command=self.next_frame).pack(side=tk.LEFT, padx=2)
        self.frame_label = tk.Label(f_frame, text="Frame: 0/0")
        self.frame_label.pack(side=tk.LEFT, padx=10)
        tk.Checkbutton(f_frame, text="Convert all frames", variable=self.convert_all_frames).pack(side=tk.LEFT, padx=15)

        self.preview_label = tk.Label(self.root, text="Preview Area", bg="gray")
        self.preview_label.pack(pady=10, expand=True, fill=tk.BOTH, padx=20)

        self.btn_convert = tk.Button(self.root, text="Generate Maps (.dat)", command=self.start_conversion,
    ...
    def browse_file(self):
        path = filedialog.askopenfilename(
            filetypes=[("All supported", "*.png *.jpg *.jpeg *.bmp *.gif *.webp *.tiff *.mp4 *.avi *.mov *.mkv *.webm"),
    ...
    def load_frames(self):
        ...
        if not path or not os.path.exists(path):
            return
        try:
            self.log_msg("Loading file...")
            self.frames_cache = load_frames_from_file(path, log_func=self.log_msg)
            self.log_msg(f"Loaded {len(self.frames_cache)} frames.")
    ...
    def preview_frame(self, idx):
        ...
        self.frame_label.config(text=f"Frame: {idx+1}/{len(self.frames_cache)}")
    ...
    def start_conversion(self):
        ...
        if not os.path.exists(out_dir):
            try:
                ...
                messagebox.showerror("Error", f"Cannot create directory: {out_dir}")
                return
        ...
        def worker():
            try:
                if not self.frames_cache:
                    callback("Loading frames...")
                    self.frames_cache = load_frames_from_file(path, log_func=callback)
                    callback(f"Loaded {len(self.frames_cache)} frames")
                ...
                frames_to_convert = []
                if do_all:
                    callback(f"Batch conversion: {len(self.frames_cache)} frames")
                    frames_to_convert = list(range(len(self.frames_cache)))
                else:
                    callback(f"Converting current preview frame: {self.preview_index+1}")
                ...
                for frame_idx in frames_to_convert:
                    ...
                    if split:
                        ...
                        id_offset += 1
                callback(f" All done, generated {id_offset} map files.")
                self.root.after(0, lambda: messagebox.showinfo("Success", f"Conversion complete!\nGenerated {id_offset} map files."))
            except Exception as e:
                callback(f" Error: {e}")
    ...
    def _convert_split(self, img, out_dir, cols, rows, start_id, dither, callback):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                callback(f"  Tile ({col+1},{row+1}) -> map_{map_id}.dat")
    ...
    def finish_conversion(self):
        self.progress.stop()
        self.btn_convert.config(state="normal", text="Generate Maps (.dat)")
